...
//Dessiner un signe égal

public class SigneEgal {
    ...
    private final short Indices[] = { 0,1,4,1,4,5,4,8,6,8,6,10,6,3,2,6,7,2,9,5,11,5,11,7,12,13,16,13,16,17,18,19,14,19,14,15 };
    ...
    public SigneEgal(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(EgalCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(EgalCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(EgalColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(EgalColors);
