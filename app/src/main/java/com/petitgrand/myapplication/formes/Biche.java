...
public class Biche extends Animal {
    ...
    private final short Indices[] = { 0, 1, 2, 2,1,3,2,3,4,3,4,5,0,6,11,0,5,11,6,7,8,7,8,9,8,9,10,10,9,11 };
    ...
    public Biche(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(bicheCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(bicheCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(bicheColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(bicheColors);
    ...
    public float getHeight(){
        return abs(bicheCoords[7])+ abs(bicheCoords[25]);
    }
}
