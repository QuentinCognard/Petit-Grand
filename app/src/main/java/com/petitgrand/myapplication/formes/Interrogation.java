/*
 * Copyright (C) 811 The Android Open Source Project
 *
...
//import android.opengl.GLES8;
import android.opengl.GLES30;

import com.petitgrand.myapplication.MyGLRenderer;


//Dessine un point d'interrogation
...
public class Interrogation {
    ...
    static float InterrogationCoords[] = {
            0.0f, -2.75f, 0.0f,
            -0.5f, -2.75f, 0.0f,
            0.5f, -2.75f, 0.0f,
            0.0f, -3.13f, 0.0f,
    ...
    static float InterrogationColors[] = {
            1f,  1f, 0.2f, 1.0f,
            1f,  1f, 0.2f, 1.0f,
            1f,  1f, 0.2f, 1.0f,
            1f,  1f, 0.2f, 1.0f,
    ...
    private final short Indices[] = {0,1,4,4,2,0,2,0,3,3,0,1,5,6,7,8,9,11,10,11,8,8,9,12,12,13,8,12,13,6,6,7,12 };
    ...
    public Interrogation(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(InterrogationCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(InterrogationCoords);
        vertexBuffer.position(0);

        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(InterrogationColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(InterrogationColors);
