...
public class Ours extends Animal {
    ...
    private final short Indices[] = { 0,1,2,1,2,3,4,5,6,5,6,7 };
    ...
    public Ours(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(oursCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(oursCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(oursColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(oursColors);
    ...
    public float getHeight(){
        return abs(oursCoords[13])+ abs(oursCoords[19]);
    }
