...
//Dessiner un cadre mystère

public class CadreMystere {
    ...
    static float MystereColors[] = {
            0.1f,  0.92f, 0.99f, 1.0f,
            0.1f,  0.92f, 0.99f, 1.0f,
            0.1f,  0.92f, 0.99f, 1.0f,
            0.1f,  0.92f, 0.99f, 1.0f,
            0.1f,  0.92f, 0.99f, 1.0f,
    ...
    private final short Indices[] = { 0,1,4,1,4,5,4,8,6,8,6,10,6,3,2,6,7,2,9,5,11,5,11,7};
    ...
    public CadreMystere(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(MystereCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(MystereCoords);
        vertexBuffer.position(0);

        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(MystereColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(MystereColors);
