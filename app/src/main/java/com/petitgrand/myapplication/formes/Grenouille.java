...
public class Grenouille extends Animal {
    ...
    static float grenouilleColors[] = {
            0.45f,  0.86f, 0.15f, 1.0f,
            0.45f,  0.86f, 0.15f, 1.0f,
            0.45f,  0.86f, 0.15f, 1.0f,
            0.45f,  0.86f, 0.15f, 1.0f,
            0.45f,  0.86f, 0.15f, 1.0f};

    private final short Indices[] = { 0, 1, 2,1,2,3,2,3,4 };
    ...
    public Grenouille(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(grenouilleCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(grenouilleCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(grenouilleColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(grenouilleColors);
    ...
    public float getHeight(){
        return abs(grenouilleCoords[1])+ abs(grenouilleCoords[4]);
    }
}
