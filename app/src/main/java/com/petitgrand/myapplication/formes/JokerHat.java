...
//Dessiner la coiffe du joker

public class JokerHat {
    ...
    static float JokerColorsHat[] = {

            0.89f,  0.07f, 0.07f, 1.0f,
            0.89f,  0.07f, 0.07f, 1.0f,
            0.89f,  0.07f, 0.07f, 1.0f,
    ...
    private final short IndicesHat[] = { 1,2,3,0,2,4,3,0,5,1,6,4 };
    ...
    public JokerHat(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(JokerCoordsHat.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(JokerCoordsHat);
        vertexBuffer.position(0);

        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(JokerColorsHat.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(JokerColorsHat);
        colorBuffer.position(0);

        // initialisation du buffer des indices
        ByteBuffer dlb = ByteBuffer.allocateDirect(IndicesHat.length * 2);
        dlb.order(ByteOrder.nativeOrder());
        indiceBuffer = dlb.asShortBuffer();
        indiceBuffer.put(IndicesHat);
    ...
    public void draw(float[] mvpMatrix) {
        ...
        GLES30.glDrawElements(
                GLES30.GL_TRIANGLES, IndicesHat.length,
