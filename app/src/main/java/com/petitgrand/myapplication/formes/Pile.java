...
public class Pile {
    ...
    static float PileCoords[] = {
            -1.0f,   0.10f, 0.0f,
            1.0f,  0.10f, 0.0f,
            -1.0f,  -0.10f, 0.0f,
            1.0f,   -0.10f, 0.0f};
    // Le tableau des couleurs
    static float PileColors[] = {
    ...
    public Pile(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(PileCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(PileCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(PileColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(PileColors);
    ...
    public float[] get_position() {
        float [] res = { Position[0],Position[1] };
        return res;
    }
    /* La fonction Display */
