...
//Dessiner un signe moins

public class SigneMoins {
    ...
    static float MoinsColors[] = {
            0.25f,  0.25f, 0.69f, 1.0f,
            0.25f,  0.25f, 0.69f, 1.0f,
            0.25f,  0.25f, 0.69f, 1.0f,
            0.25f,  0.25f, 0.69f, 1.0f,
            0.25f,  0.25f, 0.69f, 1.0f,
    ...
    private final short Indices[] = { 0,1,4,1,4,5,4,8,6,8,6,10,6,3,2,6,7,2,9,5,11,5,11,7,12,13,14,13,14,15 };
    ...
    public SigneMoins(float[] Pos) {

        Position[0] = Pos[0];
        Position[1] = Pos[1];
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(MoinsCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(MoinsCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(MoinsColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(MoinsColors);
